...
* **做法：** 构造一个与 Token Embedding 维度相同的正弦/余弦矩阵，然后**直接相加**：$X_{final} = X_{token} + PE_{absolute}$。
...
在一个二维平面上，如果你把向量 $\mathbf{q}$ 旋转角度 $m\theta$，把向量 $\mathbf{k}$ 旋转角度 $n\theta$。当计算它们的点积（夹角）时，结果只和相对角度 (m - n) 有关！
公式如下：
...
2. **核心阈值**：YaRN 通过**原始最大长度 $L$**（`orig_max`）与**波长 $\lambda_i$** 的比值 $\beta$（即代码中的 `b`）来界定边界：$\beta = \frac{L}{\lambda_i}$。
3. **反推索引**：将波长公式代入并求解维度索引 $i$：
